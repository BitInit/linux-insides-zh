...
在[上一节中](https://xinqiu.gitbooks.io/linux-insides-cn/content/Booting/linux-bootstrap-1.html)我们开始接触到内核启动代码，并且分析了初始化部分，最后我们停在了对`main`函数（`main`函数是第一个用C写的函数）的调用（`main`函数位于[arch/x86/boot/main.c](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/main.c)）。
...
让我们打开C文件 [arch/x86/boot/main.c](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/main.c)。这个文件包含了很多的函数，这些函数分别会执行键盘初始化，内存堆初始化等等操作...，下面让我们来具体看一些重要的函数。

将启动参数拷贝到"zeropage"
--------------------------------------------------------------------------------

让我们从`main`函数开始看起，这个函数中，首先调用了[`copy_boot_params(void)`](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/main.c#L30)。 

这个函数将内核设置信息拷贝到`boot_params`结构的相应字段。大家可以在[arch/x86/include/uapi/asm/bootparam.h](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/include/uapi/asm/bootparam.h#L113)找到`boot_params`结构的定义。
...
1. 将[header.S](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/header.S#L281)中定义的 `hdr` 结构中的内容拷贝到 `boot_params` 结构的字段 `struct setup_header hdr` 中。

2. 如果内核是通过老的命令行协议运行起来的，那么就更新内核的命令行指针。

这里需要注意的是拷贝 `hdr` 数据结构的 `memcpy` 函数不是C语言中的函数，而是定义在 [copy.S](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/copy.S)。让我们来具体分析一下这段代码：
...
你可以在 [arch/x86/include/asm/linkage.h](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/include/asm/linkage.h)中找到 `GLOBAL` 宏定义。这个宏给代码段分配了一个名字标签，并且让这个名字全局可用。 
...
你可以在[include/linux/linkage.h](https://elixir.bootlin.com/linux/v3.18/source/include/linux/linkage.h)中找到 `ENDPROC` 宏的定义。 这个宏通过 `END(name)` 代码标识了汇编函数的结束，同时将函数名输出，从而静态分析工具可以找到这个函数。
...
在 `hdr` 结构体被拷贝到 `boot_params.hdr` 成员之后，系统接下来将进行控制台的初始化。控制台初始化时通过调用[arch/x86/boot/early_serial_console.c](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/early_serial_console.c)中定义的 `console_init` 函数实现的。
...
`puts` 函数定义在[tty.c](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/tty.c)。这个函数只是简单的调用 `putchar` 函数将输入字符串中的内容按字节输出。下面让我们来看看  `putchar`函数的实现：
...
`__attribute__((section(".inittext")))` 说明这段代码将被放入 `.inittext` 代码段。关于 `.inittext` 代码段的定义你可以在 [setup.ld](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/setup.ld#L19)中找到。
...
下面让我们来看看[memset](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/copy.S#L36)函数的实现 :
...
在 `bisoregs` 结构体被 `initregs` 函数正确填充之后，`bios_putchar` 调用中断 [0x10](http://www.ctyme.com/intr/rb-0106.htm) 在显示器上输出一个字符。接下来 `putchar` 函数检查是否初始化了串口，如果串口被初始化了，那么将调用[serial_putchar](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/tty.c#L30)将字符输出到串口。

堆初始化
--------------------------------------------------------------------------------

当堆栈和bss段在[header.S](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/header.S)中被初始化之后 (细节请参考上一篇[part](linux-bootstrap-1.md))， 内核需要初始化全局堆，全局堆的初始化是通过 [`init_heap`](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/main.c#L116) 函数实现的。

代码首先检查内核设置头中的[`loadflags`](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/header.S#L321)是否设置了 [`CAN_USE_HEAP`](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/include/uapi/asm/bootparam.h#L21)标志。 如果该标记被设置了，那么代码将计算堆栈的结束地址：:
...
在堆栈初始化之后，内核代码通过调用[arch/x86/boot/cpu.c](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/cpu.c)提供的 `validate_cpu` 方法检查CPU级别以确定系统是否能够在当前的CPU上运行。

`validate_cpu` 调用了[`check_cpu`](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/cpucheck.c#L102)方法得到当前系统的CPU级别，并且和系统预设的最低CPU级别进行比较。如果不满足条件，则不允许系统运行。
...
接下来，内核调用 `detect_memory` 方法进行内存侦测，以得到系统当前内存的使用分布。该方法使用多种编程接口，包括 `0xe820`（获取全部内存分配），`0xe801` 和 `0x88`（获取临近内存大小），进行内存分布侦测。在这里我们只介绍[arch/x86/boot/memory.c](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/memory.c)中提供的 `detect_memory_e820` 方法。
...
接下来内核调用[`keyboard_init()`](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/main.c#L65) 方法进行键盘初始化操作。 首先，方法调用`initregs`初始化寄存器结构，然后调用[0x16](http://www.ctyme.com/intr/rb-1756.htm)中断来获取键盘状态。
...
[query_mca](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/mca.c#L18) 方法调用[0x15](http://www.ctyme.com/intr/rb-1594.htm)中断来获取机器的型号信息，BIOS版本以及其他一些硬件相关的属性：
...
在[boot.h](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/boot.h) 存在很多类似于 `set_fs` 的方法, 比如 `set_gs`。
...
接下来，内核会调用[query_apm_bios](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/apm.c#L21) 方法从BIOS获得 [高级电源管理](http://en.wikipedia.org/wiki/Advanced_Power_Management) 信息。`query_apm_bios` 也是调用 `0x15` 中断，只不过将 `ax` 设置成 `0x5300` 以得到APM设置信息。中断调用返回之后，代码将检查 `bx` 和 `cx` 的值，如果 `bx` 不是 `0x504d` ( PM 标记 )，或者 `cx` 不是 `0x02` (0x02，表示支持32位模式)，那么代码直接返回错误。否则，将进行下面的步骤。
...
最后是[`query_edd`](https://elixir.bootlin.com/linux/v3.18/source/arch/x86/boot/edd.c#L122) 方法调用, 这个方法从BIOS中查询 `Enhanced Disk Drive` 信息。下面让我们看看 `query_edd` 方法的实现。

首先，代码检查内核命令行参数是否设置了[edd](https://elixir.bootlin.com/linux/v3.18/source/Documentation/kernel-parameters.txt#L1023) 选项，如果edd选项设置成 `off`，`query_edd` 不做任何操作，直接返回。
...
在代码中 `0x80` 是第一块硬盘，`EDD_MBR_SIG_MAX` 是一个宏，值为16。代码把获得的信息放入数组[edd_info](https://elixir.bootlin.com/linux/v3.18/source/include/uapi/linux/edd.h#L172)中。`get_edd_info` 方法通过调用 `0x13` 中断调用（设置 `ah = 0x41` ) 来检查EDD是否被硬盘支持。如果EDD被支持，代码将再次调用 `0x13` 中断，在这次调用中 `ah = 0x48`，并且 `si` 指向一个数据缓冲区地址。中断调用之后，EDD信息将被保存到 `si` 指向的缓冲区地址。
...
* [earlyprintk documentation](https://elixir.bootlin.com/linux/v3.18/source/Documentation/x86/earlyprintk.txt)
* [Kernel Parameters](https://elixir.bootlin.com/linux/v3.18/source/Documentation/kernel-parameters.txt)
* [Serial console](https://elixir.bootlin.com/linux/v3.18/source/Documentation/serial-console.txt)
